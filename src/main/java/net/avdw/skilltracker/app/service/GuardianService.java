...
public class GuardianService implements GuardianQuery {
    ...
    @Inject
    public GuardianService(AllyRepo allyRepo) {
        this.allyRepo = allyRepo;
    }

    @Override
    public Optional<Player> findGuardian(Player player) {
    ...
    @Override
    public List<Player> findWards(Player player) {
        return allyRepo.findAll(player).stream()
                .map(Ally::getAlly)
                .filter(ally -> findGuardian(ally).stream().anyMatch(player::equals))
